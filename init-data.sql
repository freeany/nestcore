...
INSERT INTO users (id, username, email, password, isActive, lastLoginAt, createdAt, updatedAt) VALUES
...
-- 插入用户资料数据
INSERT INTO profiles (id, firstName, lastName, realName, nickname, phone, birthday, gender, address, bio, avatar, company, position, website, wechat, qq, weibo, user_id, createdAt, updatedAt) VALUES
...
-- 插入操作日志数据
INSERT INTO operation_logs (id, action, module, description, oldData, newData, ipAddress, userAgent, status, errorMessage, user_id, createdAt) VALUES
...
SELECT '=== 用户数据 ===' as info;
SELECT id, username, email, isActive, lastLoginAt FROM users;
...
SELECT id, realName, nickname, phone, gender, company, position, user_id FROM profiles;

SELECT '=== 操作日志数据 ===' as info;
SELECT id, action, module, description, status, user_id, createdAt FROM operation_logs ORDER BY createdAt DESC;
